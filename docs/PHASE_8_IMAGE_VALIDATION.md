...
Der Google-Zugang ist eingerichtet. Am 1. September 2026 wurde nach getrennten Einwilligungen genau ein freigegebener Fall F01 ausgeführt: Der serverseitige Datensatz meldet `succeeded`, 18.104 ms Laufzeit und 30 Cent Reservierung. Externe KI wurde danach wieder ausgeschaltet. Der tatsächliche Google-Rechnungsbetrag bleibt bis zum Abgleich `null`; Bild und Zuordnungsdaten bleiben privat außerhalb von Git. Eine menschliche Qualitätsbewertung und der Abrechnungsabgleich stehen noch aus. Diese Punkte können nicht durch Softwaretests ersetzt werden.
