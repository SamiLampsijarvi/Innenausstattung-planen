...
Die erste Zustimmung vom 31.08.2026 erlaubte Implementierung und Offline-Prüfung. Die nachfolgende Freigabe nach Erläuterung der Supabase-Kennzeichnung „main Production“ erlaubt auch die Übernahme in das bestehende Entwicklungsprojekt und nach erfolgreichen Prüfungen nach GitHub-`main`. Google-Projekt-/Abrechnungseinrichtung, echte Fotoübertragung und Veröffentlichung bleiben ausgeschlossen. Die beiden serverseitigen Schalter bleiben standardmäßig `false`; die Datenbankkampagne bleibt gesperrt und ohne geprüften Preis.

Der vorherige Prüfabschnitt umfasste die isolierte Supabase-Integrationsprüfung und das Speichern des Entwicklungsbranches auf GitHub. Dafür wurde der bestehende Ubuntu-Testjob des öffentlichen Repositories verwendet; kein neues Supabase-Cloudprojekt und keine Google-Ausführung. Die später gesondert freigegebene Datenbankübernahme ist unten dokumentiert.
...
7. Die gesondert freigegebene Migration und erste Betriebsprüfung wurden am 31.08.2026 abgeschlossen (siehe unten). Vor einem echten Test die Prüfung wiederholen und nur das benannte Testkonto in `image_test_members` aufnehmen. Servergeheimnis lokal außerhalb Git setzen; beide KI-Schalter bleiben zunächst aus.
...
- Vor echtem Versand weiterhin offen: benanntes Testkonto, sichere lokale Serverkonfiguration, erneute Betriebsprüfung und laufende Überwachung; Google-Zugang, Preisobergrenze, Datenschutz und gesonderte Ausführungsfreigabe. Kein Erfolg einer echten Bildgenerierung behauptet.

## Übernahme in das bestehende Entwicklungsprojekt am 31.08.2026

Nach ausdrücklicher Freigabe trotz der Supabase-Kennzeichnung „main Production“ wurde ausschließlich `202608310001_controlled_image_test.sql` in „HOMERA - project“ übernommen. Die acht vorhandenen Migrationen entsprachen dem Repository. Migration und Eintrag in der Migrationshistorie wurden gemeinsam in einer Transaktion ausgeführt, mit fünf Sekunden Sperrwartezeit und 30 Sekunden Abfragezeitlimit. Bestehende Konten, Projekte und Fotos wurden nicht bearbeitet. Es wurde kein neuer Tarif, Cloudzugang oder Servergeheimnis eingerichtet.

Die nur lesende Betriebsabnahme `supabase/checks/phase7_acceptance.sql` bestätigte um 09:51:29 UTC: fünf Tabellen mit Zeilenzugriffsschutz, keine direkten Tabellenrechte für öffentliche/Browser-/Serverrollen, keine für Browserrollen aufrufbaren Testfunktionen und keine unsichere Funktionskonfiguration. Die Datenbankgrenzen für fünf Fotos, zwei Versuche und 300 Cent sind vorhanden. Kampagne deaktiviert, keine Mitglieder, Fotos, Versuche oder Ergebnisse, null reservierte Cent und kein geprüfter Preis.

Der reale Löschjob wurde bei leerer Ergebnistabelle kurz auf minütlich gestellt und lief um 09:51:00 UTC erfolgreich. Anschließend wurde der ursprüngliche aktive Zeitplan `17 * * * *` wiederhergestellt und geprüft. Dieser Nachweis bestätigt die Ausführung im Zielprojekt; das tatsächliche Entfernen künstlicher Bilder wurde im isolierten Stack geprüft. Im verbundenen Projekt wurden keine Testbilder angelegt oder gelöscht.

Ein zusätzlicher transaktionaler Schreibtest im Zielprojekt wurde von der automatischen Sicherheitsprüfung vor Ausführung blockiert, weil er ein unbeschränktes UPDATE enthält. Er wurde nicht ausgeführt und nicht umgangen. Die Verhaltenstests einschließlich Konkurrenz, Widerruf, Kontingenten und physischer Löschung bleiben auf den isolierten vollständigen Supabase-Stack begrenzt. Die Zielprojekt-Abnahme ist bewusst nur lesend; allein der eng begrenzte Zeitplantest änderte vorübergehend den neuen Löschjob.

Der freigegebene Vorbereitungsabschnitt endet mit dieser Betriebsabnahme und erfolgreichen GitHub-Prüfungen. Er ist keine Freigabe für den echten Vertex-Test oder eine öffentliche Veröffentlichung. Vor Aktivierung erneut `supabase/checks/phase7_acceptance.sql` ausführen; während eines aktiven Tests täglich und vor jeder weiteren Versuchsgenehmigung die erfolgreichen stündlichen Läufe prüfen. Fehlen aktuelle erfolgreiche Läufe oder treten Fehler auf, keine Bildgenerierung freigeben und den Löschbetrieb zuerst reparieren. Eine automatische externe Alarmierung ist noch nicht eingerichtet.
