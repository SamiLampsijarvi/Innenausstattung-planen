...
8. Für die getrennte Einwilligung und Fotozulassung darf ausschließlich `RAUMLY_IMAGE_TEST_ENABLED=true` gesetzt werden. `RAUMLY_IMAGE_AI_ENABLED` bleibt aus. Im internen Bereich Foto und Planungswerte kontrollieren.
...
- PGlite verarbeitet überlappende Anfragen serialisiert. Das ersetzt keinen Mehrverbindungstest auf einem echten PostgreSQL-Server. Ein solcher Test muss vor Aktivierung zwei parallele Reservierungen und konkurrierenden Widerruf gegen dieselbe Kampagne prüfen: höchstens eine erfolgreiche Buchung, keine Überschreitung und nach Widerruf keine Ergebnisfreigabe.
- `supabase test db`: zusätzlicher pgTAP-Test für den echten lokalen Supabase-Stack samt Zeitplanregistrierung. Dieser Stack war auf diesem Rechner nicht verfügbar; die CI-Konfiguration enthält bereits den Supabase-Testjob. Keine Ausführung gegen das verbundene Projekt erfolgt.
...
- Vor echtem Versand offen: integrierter Auth-/RPC-/Storage-Test auf Supabase, tatsächliche Mehrverbindungsprüfung, Löschjobausführung/Überwachung, Google-Zugang, Preisobergrenze, Datenschutz und gesonderte Freigabe. Kein Erfolg einer echten Bildgenerierung behauptet.
