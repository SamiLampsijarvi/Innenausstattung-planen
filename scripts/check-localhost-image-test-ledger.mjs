...
const response = await fetch(`${values.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/rpc/localhost_image_test_ledger_status`, {
  method: "POST",
  headers: {
...
if (!response.ok) throw new Error(`Testbuchhaltung nicht verfügbar (HTTP ${response.status}).`);

const ledger = await response.json();
if (!ledger || typeof ledger !== "object") throw new Error("Ungültige Antwort der Testbuchhaltung.");
