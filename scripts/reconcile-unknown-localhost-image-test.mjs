...
const response = await fetch(`${values.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/rpc/localhost_image_test_reconcile_unknown_attempt`, {
  method: "POST",
...
if (!response.ok) throw new Error(`Ungeklärter Versuch konnte nicht sicher abgeglichen werden (HTTP ${response.status}).`);
