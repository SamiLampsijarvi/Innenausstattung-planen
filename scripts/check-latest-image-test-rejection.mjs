...
const response = await fetch(`${values.NEXT_PUBLIC_SUPABASE_URL}/rest/v1/rpc/localhost_image_test_latest_rejection_diagnostics`, {
  method: "POST",
  headers: {
...
if (!response.ok) throw new Error(`Raumtreue-Diagnose nicht verfügbar (HTTP ${response.status}).`);
