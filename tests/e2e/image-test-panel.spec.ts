...
test("Testoberfläche trennt Einwilligung, Fotozulassung und kostenpflichtigen Start", async ({ page, context }) => {
  ...
  const state = {
    ...
    photos: [] as { id: string; photo_id: string; attempts: number; style: string; budget_euro: number }[], attempts: [],
  };
  ...
  await page.route("**/api/internal/image-test", async (route) => {
    if (route.request().method() === "POST") {
      ...
      if (body.action === "approve") {
        state.photos = [{ id: "cccccccc-cccc-4ccc-8ccc-000000000001", photo_id: body.photoId, attempts: 0, style: "Japandi", budget_euro: 1500 }];
  ...
  await expect(page.getByRole("checkbox")).not.toBeChecked();
  expect(actions).toEqual(["grant", "approve", "withdraw"]);
});
