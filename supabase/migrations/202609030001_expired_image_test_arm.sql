-- An approval window authorizes dispatch only until its expiry.  If no
-- reservation was created, a later operator arm must safely replace it.
create or replace function public.image_test_arm(
...
begin
  ...
  if c.enabled and c.active_attempt is null and c.approved_until <= now() then
