...
export const LOCAL_PROJECTS_VERSION = 2;
...
export type LivingRoomPlan = {
  style: string; postcode: string; budget: number; furnitureReview: FurnitureReview;
};
...
const emptyPlan = (): LivingRoomPlan => ({ style: "", postcode: "", budget: 1500, furnitureReview: emptyFurnitureReview() });
...
function migrateVersionOneProject(value: unknown): LocalProject | null {
  ...
  return {
    ...
    livingRoom: { style: plan.style, postcode: plan.postcode, budget: plan.budget, furnitureReview: emptyFurnitureReview() },
  };
}
...
function isLocalProject(value: unknown): value is LocalProject {
  ...
  return typeof project.id === "string" && typeof project.name === "string"
    ...
    && review.items.every(isFurnitureItem);
}
